...
1. [X] `npm run release:check` — `php -l`/`bash -n` pass (blocking); `phpcs`/`phpstan`
       output reviewed, no unexplained new findings (informative)
2. [X] PHPUnit unit suite (`npm run test:unit`) **green**, no skipped or risky tests
3. [X] E2E suite (see the prerequisite for the `Phase 5 - Validation` in the
       `docs/release-preparation.md` then the E2E suite can be started
       `cd tests/e2e && npm test`, see `tests/e2e/README.md`) **all
...
1. [X] wp-env start
2. [X] Activate the plugin; confirm Settings → OpenPorte Anti-spam shows "API Mode:
       Self-hosted" by default
3. [X] Enable an integration (e.g. WordPress → Comments)
4. [X] Submit a comment with the OpenPorte widget — confirm the entry is created; no PHP
       errors in wp-env logs
5. [X] Also verify there are no outgoing network requests to altcha.org in the browser's
...
1. [X] In Settings → OpenPorte Anti-spam, change "API Mode" to "Custom"
2. [X] Confirm the "Challenge URL" text input becomes enabled immediately (JS toggle)
3. [X] Enter any URL; confirm it saves; confirm the widget's challengeurl attribute
       reflects it in the rendered HTML
4. [X] Negative test: in Self-hosted mode, confirm that the Challenge URL field is
...
1. [X] After each action above, run wp-env logs and confirm no PHP Warning, PHP Notice,
...
2. [X] After each action above, check in the browser development console that there are
       no warning or error.

**(d) Upgrade successful**

1. [X] Test a legacy install: before wp-env start, manually set altcha_api to "eu" in
...
2. [X] ALTCHA → OpenPorte migration on a bench with ALTCHA v1 previously configured (see
       `tests/README.md`): settings carried over, forms keep verifying
3. [X] Algorithm default on upgrade — all four routes, since only a fresh install may
       end up on SHA-512 (`wp option get openporte_algorithm`, and confirm the served
       challenge's `algorithm` field agrees):
       - [X] Fresh activation on a clean bench → seeded **SHA-512**
       - [X] Plugin update from pre-1.28 OpenPorte (activation hook does not run, so no
             option is stored) → `get_algorithm()` falls back to **SHA-256**
       - [X] ALTCHA v1 → OpenPorte install + activate (the activation hook *does* run,
             right after the legacy migration) → **SHA-256**
       - [X] Deactivate + reactivate a pre-1.28 OpenPorte bench → still **SHA-256**,
...
4. [X] A bench upgraded from a version with spam-filter options set shows no notice and
       silently ignores the leftover options
5. [X] Replay limit is seeded on **both** routes and never overwrites a choice
       (`wp option get openporte_replaylimit`):
       - [X] Fresh activation on a clean bench → **5**
       - [X] Plugin update from 1.28.x (the activation hook does not run, so
             `openporte_upgrade()` seeds it on `plugins_loaded`) → **5**
       - [X] Set it to 1, then deactivate + reactivate → still **1**, i.e. a plugin
             toggle must not reset an admin's choice
6. [X] No expiry migration happened: a bench with `openporte_expires` set to `0` or `30`
       still holds that value after the upgrade (1.29.0 warns, it does not migrate)

**(e) PHP and WordPress compatibility verification**

1. [X] Test with PHP 8.0 and WordPress 5.6 — minimum supported combo (PHP floor, first
       WP version with PHP 8.0 support)
2. [X] Test with PHP 8.3 and WordPress 6.8 — oldest maintained PHP version, the WP major
       before current stable
3. [X] Test with PHP 8.5 and WordPress 7.1 — newest PHP version, newest WP version available
...
1. [X] Default limit (5): solve a challenge, capture the token, and replay it —
       accepted five times in total, refused on the sixth
2. [X] Strict (1): a replay is refused immediately, and **logging in still works** on
       both `wp-login.php` and the WooCommerce login form (the dual `authenticate`
       registration — the highest-harm failure mode)
3. [X] Unlimited (0): repeated replays are all accepted, and a query for
       option names matching `%openporte_replay_%` returns **nothing** — the
       counter must write no state at all when it is off
4. [X] Counter storage and lifetime, database path (no object cache): after one
...
6. [ ] **Concurrency — verified by design for this release, not empirically.**
       ...
       - [x] **The claim is one statement, not a read-then-write.**
             ...
             Confirmed by reading `includes/core.php` (bench 2026-09-09,
             WordPress 7.1, PHP 8.5, MariaDB 11.8): the guarded `UPDATE` is a
             single statement, unchanged since the last review
       - [x] **`add_option()` is not used for the claim.** Core implements it as
             `INSERT … ON DUPLICATE KEY UPDATE` behind a cached existence check,
             which is exactly the lost update this design must avoid.
             Confirmed: the value row is claimed with a direct `$wpdb->query('INSERT
             IGNORE …')`, not `add_option()` — only the timeout row (not the
             claim itself) uses `add_option()`
       - [x] **The object-cache path uses `INCR`, not get-then-set** —
             `wp_cache_add()` to seed, `wp_cache_incr()` to claim, with the
             result compared against the limit. Confirmed by reading
             `consume_replay_slot()`: `wp_cache_add()` seeds `'0'` (string, to
             avoid a serialising drop-in silently breaking `INCR`), then
             `wp_cache_incr()` claims and the returned count is compared to the
             limit — no read-then-set
...
7. [X] Fail-open is observable: stop the object cache (or otherwise break the store),
       ...
       No persistent object cache is installed on this bench (database path), so the
       store was broken surgically instead: a temporary mu-plugin hooked the `query`
       filter to no-op only the replay counter's own INSERT/UPDATE statements (matched
       on the option-name substring), leaving every other query — including the
       counter's own read-back and the health-record write — untouched. Result: the
       submission was accepted (comment created), **zero** new
       `_transient_openporte_replay_*`/`_transient_timeout_*` rows were written (the
       replay limit was strict (1) and would otherwise have refused it), and
       `openporte_evaluate_replay_protection()` — the exact function the settings
       page calls — returned the fail-open warning naming the degradation. The
       `openporte_replay_store_unavailable` listener fired with the expected
       `$key`/`$limit`/`$ttl`. Mu-plugin removed and test rows cleaned up afterward.
8. [X] Deprecation notice: with `WP_DEBUG` on, a direct call to
       `OpenPortePlugin::$instance->verify_solution($token)` logs a deprecation naming
       `verify()`; a normal form submission logs **nothing**.
       `WP_DEBUG`/`WP_DEBUG_LOG` were already on. A direct `verify_solution($token)`
       call logged exactly `Deprecated: Function OpenPortePlugin::verify_solution is
       deprecated since version 1.29.0! Use verify() instead.`; a subsequent normal
       comment submission through `verify()` added **zero** bytes to `debug.log`.
...
1. [X] Replay limit field: presets save and round-trip; choosing "Custom" reveals the
       ...
       to a preset
2. [X] Sanitiser: entering `-5` stores **0** (not 5), and `250` stores **100**
       *Note (tester): entering a negative value sanitize it to 5 (so -8 => 5; -1 => 5),
       not as specified, but accepted with comment.
3. [X] Expiration advisories (Self-hosted): `0` shows a red error-style notice and
       **still saves**; a value under 60 s shows a warning and still saves; a normal
       value shows neither
4. [X] Custom mode: the Expiration **and Complexity** controls are greyed out, each
...
5. [X] Switching API Mode back and forth enables/disables Expiration, Complexity and
...
6. [X] Custom-mode health check, three outcomes and they must not be confused:
...
7. [X] Replay-protection status notice appears on the settings screen and matches
       reality (limit in force, object cache vs database)

**(h) v1.28.0 feature checks (regression)**

1. [X] Algorithm setting: Settings → OpenPorte Anti-spam offers SHA-256 / SHA-384 / SHA-512;
       select a non-default value, confirm the served challenge uses it and a submission
       still verifies end-to-end
2. [X] Expiration setting: the preset select saves; choosing "Custom" reveals the
...
3. [X] Challenge URL health check (Custom mode): on the settings screen, a reachable
...
4. [X] Custom HTML integration: its toggle is styled/labelled **Deprecated** (removal
       next major, pointing at the `[openporte]` shortcode) but remains functional when enabled
5. [X] Widget Customisation controls: aligned and consistently labelled; each option
       (hide footer/logo, auto-verification, floating, delay) still round-trips through save
6. [X] Activation-conflict screen: with ALTCHA v1 active, activating OpenPorte via
       Plugins → Add New → Upload shows the conflict screen and its link back to the
       Plugins page works (#65)

**(i) WordPress Plugin Check**

1. [X] Run the Plugin Check plugin against the build on a WP 6.3+ bench: every finding
